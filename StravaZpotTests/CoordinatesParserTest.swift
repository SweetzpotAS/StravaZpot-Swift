...
class CoordinatesParserTest : XCTestCase {
    ...
    func testCoordinatesAreConvertedToDictionary() {
        let parser = CoordinatesParser()
        let coordinates = Coordinates(latitude: 23.6, longitude: -42.1)
        
        let result = parser.toDictionary(coordinates)
        let expected = [23.6, -42.1]
        
        expect(result["coordinates"] as? [Double]).to(equal(expected))
    }
}
